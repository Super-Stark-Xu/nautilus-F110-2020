...
class PurePursuit:
	...
	location[0] = msg.pose.pose.position.x
	location[1] = msg.pose.pose.position.y
	location[2] = msg.pose.pose.orientation.z
	...
	for i in self.path_points
		d = self.dist(self, location, self.path_points[i]) 
		if d > self.LOOKAHEAD_DISTANCE
			cloest_point = seld.path_points[i] 
			self.path_points = self.path_points[i+1,end]# update way point list
			break

        # 3. Transform the goal point to vehicle coordinates. 
        goal_point[0] = cloest_point[0] - location[0]
	goal_point[1] = cloest_point[1] - location[1]   
        
        # 4. Calculate the curvature = 1/r = 2x/l^2
        # The curvature is transformed into steering wheel angle and published to the 'drive_param' topic.
	angle = 2*abs(goal_point[1])/d/d
